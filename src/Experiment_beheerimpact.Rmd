...
plot1 <- df %>% 
  ...
  labs(fill = "groot of kein")
                     
plot1
filenamfill_()filename = paste("Categorieën uitbreiding",
                 ".jpg", sep = "")
...
```{r}
library(marginaleffects)
# interval_jaar is altijd 3 in df, dus is constante en mag weg
# model aangepast om te passen bij de data, daarom ander model
m0_zonder_interactie <- glmmTMB::glmmTMB( #GLM-model
  opp_m2_2122/opp_m2_18 ~ log(opp_m2_18) + beheer, #log oppervalkte in interactie met beheer
  dispformula = ~ beheer, #beheer heeft een andere variantie
  family = Gamma(link = "log"), #data volt een gamma distributie
  data = df)

m0_zonder_interactie_zonder_disp <- glmmTMB::glmmTMB( #GLM-model
  opp_m2_2122/opp_m2_18 ~ log(opp_m2_18) + beheer, #log oppervalkte in interactie met beheer
  #dispformula = ~ beheer, #beheer heeft een andere variantie
  family = Gamma(link = "log"), #data volt een gamma distributie
  data = df)
...
m0 <- glmmTMB::glmmTMB( #GLM-model
  opp_m2_2122/opp_m2_18 ~ log(opp_m2_18) * beheer, #log oppervalkte in interactie met beheer
  dispformula = ~ beheer, #beheer heeft een andere variantie
  family = Gamma(link = "log"), #data volt een gamma distributie
  data = df)

AIC(m0_zonder_interactie, m0, m0_zonder_interactie_zonder_disp)


performance::check_residuals(m0) |> plot() #residuelen moeten normaal verdeeld zijn
summary(m0)
car::Anova(m0) #interactie tussen beginoppervlakte en beheer is significant

p0 <- predictions(
  m0,
  newdata = datagrid(
    opp_m2_18 = c(1 , 2, 5, 10, 20, 50, 100, 150, 500, 1000), #reeks predicties voor verschillende oppervlaktes
    beheer = c("nulbeheer", "maaibeheer") 
  ),
  type = "link"
) %>%
  # verwijder combinaties die niet voorkomen in de dataset
  filter(
    !(beheer == "maaibeheer" & !between(opp_m2_18, 1, 150))
  )

p0 %>%
  as_tibble() %>%
  mutate(
    ratio.est = exp(estimate) ^ (1/3), #terugrekenen naar niet-log en voor 1 jaar
    ratio.lwr = exp(conf.low) ^ (1/3),
    ratio.upr = exp(conf.high) ^ (1/3)
  ) %>%
  ggplot() +
  geom_ribbon(
    aes(
  ...
  scale_y_continuous(
    ...
    breaks = c(1/2, 3/4, 1, 5/4, 3/2, 2, 3),
    transform = "log",
    labels = scales::percent
  ) +
  scale_x_log10(
    "Initiële oppervlakte (m²)"
  )
...
p0 %>%
  ...
  filter(opp_m2_18 %in% c(1, 5, 10, 50, 100, 500, 1000)) %>%
  ggplot() +
  geom_pointrange(
    aes(
      x = factor(opp_m2_18),
  ...
  geom_hline(yintercept = 1) +
  scale_y_continuous(
    "Jaarlijkse verandering oppervlakte",
    breaks = c(1/2, 3/4, 1, 5/4, 3/2, 2, 3),
    transform = "log",
    labels = scales::percent
  ) +
  labs(x = "Beginoppervlakte (m²)")
ggsave(file = "2025_jaarlijkse_groeisnelheid_ifvbeheer_modeloutput.jpg",
       width = 6,
       height = 4,
       dpi = 300)
...
# ipv logratio als response
# voorspelling maken van opp in 2021/22 als functie van 
# initiële opp in interactie met beheer
# opp initieel log getransformeerd
# want opp gemodelleerd via Gamma met log link
# geeft net hetzelfde resultaat mits je het omrekent naar ratios
m1 <- glmmTMB::glmmTMB(
  opp_m2_2122 ~ log(opp_m2_18) * beheer,
  dispformula = ~ beheer,
  family = Gamma(link = "log"),
  data = df)


performance::check_residuals(m1) |> plot()
summary(m1)
car::Anova(m1)
...
p1 <- predictions(
  m1,
  newdata = datagrid(
    opp_m2_18 = c(1 , 2, 5, 10, 20, 50, 100, 150, 500, 1000),
    beheer = c("nulbeheer", "maaibeheer")
  ),
  type = "link"
) %>%
...
p1 %>%
  as_tibble() %>%
  mutate(
    ratio.est = exp(estimate - log(opp_m2_18)) ^ (1/3),
    ratio.lwr = exp(conf.low - log(opp_m2_18)) ^ (1/3),
    ratio.upr = exp(conf.high - log(opp_m2_18)) ^ (1/3)
  ) %>%
  ggplot() +
  geom_ribbon(
    aes(
  ...
  geom_hline(yintercept = 1) +
  scale_y_continuous(
    "Jaarlijkse groeisnelheid (200% = x 2; 50% = x 1/2)",
    breaks = c(1/2, 3/4, 1, 5/4, 3/2, 2, 3),
    transform = "log",
    labels = scales::percent
  ) +
